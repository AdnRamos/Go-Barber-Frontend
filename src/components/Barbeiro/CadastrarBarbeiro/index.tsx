...
const CadastrarBarbeiro = () => {
  ...
  const { mutate } = useMutation(
    ...
    {
      onSuccess: () => {
        push(APP_ROUTES.private.barbeiros.name); // Ajuste a rota conforme necessário
      },
      onError: (error) => {
    ...
    <>
      <div className={style.header}>
        ...
        <div className={style.header__navegacao}>
          <div className={style.header__navegacao_voltar} onClick={() => push(APP_ROUTES.private.barbeiros.name)}>
            <img src="/assets/icons/menor_que.svg" alt="Voltar" />
            <h1>Voltar</h1>
          </div>
          <div className={style.header__navegacao_guia}>
            <span>Home / Barbeiros /</span><h1>Cadastrar Barbeiro</h1>
          </div>
        </div>
      </div>
